...
$softwareGrid.Columns.AddRange(
        (New-Object System.Windows.Forms.DataGridViewTextBoxColumn -Property @{HeaderText="软件名称"; DataPropertyName="DisplayName"; SortMode="Automatic"}),
        (New-Object System.Windows.Forms.DataGridViewTextBoxColumn -Property @{HeaderText="版本"; DataPropertyName="DisplayVersion"; SortMode="Automatic"; Width=80}),
        (New-Object System.Windows.Forms.DataGridViewTextBoxColumn -Property @{HeaderText="发布者"; DataPropertyName="Publisher"; SortMode="Automatic"}),
        (New-Object System.Windows.Forms.DataGridViewTextBoxColumn -Property @{HeaderText="安装路径"; DataPropertyName="InstallLocation"; SortMode="Automatic"}),
        (New-Object System.Windows.Forms.DataGridViewTextBoxColumn -Property @{HeaderText="注册表位置"; DataPropertyName="RegistryPath"; SortMode="Automatic"})
)
            ...
            foreach ($item in $items) {
                if (-not [string]::IsNullOrEmpty($item.DisplayName)) {
                    $software = [PSCustomObject]@{
                        ...
                        DisplayVersion = if (-not [string]::IsNullOrEmpty($item.DisplayVersion)) { $item.DisplayVersion } else { "未知" }
                        Publisher = if (-not [string]::IsNullOrEmpty($item.Publisher)) { $item.Publisher } else { "未知" }
                        InstallLocation = if (-not [string]::IsNullOrEmpty($item.InstallLocation)) { $item.InstallLocation } else { "未知" }
